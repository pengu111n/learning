...
public class Ex10Operator04 {
    public static void main(String[] args) {
        ...
        // 대표적으로 어떤값이 특정범위에 속하는지 연산하는 경우,
        // 우리가 반드시 논리연산자를 사용해야한다.
        // 0 <= x <= 10
        int x = 5;
       // System.out.println(0 <= x <= 10);
        // 위에 코드는 먼저 0 <= x 가 실행되어서
        // true 혹은 false의 값을 가진 boolean 데이터타입의 결과값이 나오게된다
        // 하지만 그후에 해당 결과값을 10보다 작거나 같은지 비교할려고한다면
        // true나 false를 10과 비교할 수 없으므로
        // 위 코드는 에러가 발생한다.
        // 따라서 우리는 위의 코드를 논리연산자를 사용하여
        // x가 0이상 10이하인지를 확인해야만 한다.
        
        // 특정 범위에 속한다는 것은
        // x가 해당 범위에 최소값 보다 크다
        // x가 해당 범위의 최대값 보다 작다
        // 이두가지 조건을 모두 만족하므로, 즉 모두 true가 나올때에만
        // true가 나와야 하므로
        // 우리는 and 연산자를 사용하여
        // 해당 조건이 맞는지를 확인해야 한다.
