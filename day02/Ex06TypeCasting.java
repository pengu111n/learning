...
// 단, 데이터손실이 발생되면
// 실수의 소숫점 이하가 사라지거나
// 정수의 경우에는 overflow 혹은 underflow가 발생할 수있다

// overflow : 정수가 표현가능한 최대값보다 큰 값이 저장되면서
//           값이 오히려 음의 정수로 바뀌는 경우
// underflow : 정수가 표현 가능한 최소값보다 작은 값이 저장되면서
//             값이 오히려 양의 정수로 바뀌는 경우

public class Ex06TypeCasting {
    public static void main(String[] args) {
        // byte 변수 myByte 선언후 50 저장
        byte myByte = 50;
        
        // 암시적 형변환의 경우
        // 1. 더 작은 데이터타입의 값을 더 큰 데이터타입의 공간에 저장하는 경우
        // int 변수 myInt 선언후 myByte의 현재값을 저장
        int myInt = myByte;
        // 화면에 myInt 값 출력
        System.out.println(myInt);
        
        // 2. 정수형 데이터타입의 값을 실수형 데이터타입으로 변환하는 경우
        // double 변수 myDouble 선언후 myByte의 현재값을 저장
        ...
        // 1. 더큰 데이터타입의 값을 더 작은 데이터타입의 값으로 변환하는 경우
        // myInt의 값을 byte로 형변환하여 myByte에 저장
        myByte = (byte)myInt;
        //myByte의 값 출력
        System.out.println(myByte);
        
        // 2. 실수형 데이터타입의 값을 정수형 데이터타입의 값으로 변환하는 경우
        // myDouble의 값을 byte로 형변환하여 myByte에 저장
