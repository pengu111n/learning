...
//우리가 실행 버트을 누르면 이클립스가 자동으로 컴파일을 해주기 때문에
// 별도로 컴파일을 해줄 필요는 없다.

// 주석이란, 해당 소스파일을 컴파일 할 때
// 실행 가능한 코드가 아니라 단순한 설명으로 인식되어서
// 반환이 되지 않는 글자들을 우리가 주석이라고 한다.

// 주석에는 우리가 주로 해당코드에 대한 설명을 적게된다.

// 주석에는 크게 한줄 주석과 여러줄 주석이 있는데
// 한줄 주석은 // 가 나온 이후부터 그 줄이 끝나기 전까지 모두 주석으로 처리한다.
